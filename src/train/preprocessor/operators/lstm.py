...
class LstmPreprocessor(BasePreprocessor):
    def __init__(self, config, logger, raw_data):
        super(LstmPreprocessor, self).__init__(config, logger, raw_data)
    ...
    def process(self):
        ...
        # 调用处理形状的工具函数
        shapes = tensor_shape_split(raw_data, "input_shape")
        other_features = raw_data[["input_size", "hidden_size", "num_layers", "avg_temperature", "avg_gpu_memory_used"]]
